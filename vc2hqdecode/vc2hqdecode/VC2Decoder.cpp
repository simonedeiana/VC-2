...
void VC2Decoder::Decode(JobData *job, uint16_t **_odata, int *_ostride) {
  ...
  for (int c = 0; c < C; c++) {
    ...
    for (l = 0; l < (int)mParams.transform_params.wavelet_depth - 1; l++) {
      {
        vc2hq_stage_profile::Scope profile(vc2hq_stage_profile::INVERSE_VERTICAL);
      ...
      {
        vc2hq_stage_profile::Scope profile(vc2hq_stage_profile::INVERSE_HORIZONTAL);
...

    {
      ...
      {
        vc2hq_stage_profile::Scope profile(vc2hq_stage_profile::FINAL_OUTPUT);
