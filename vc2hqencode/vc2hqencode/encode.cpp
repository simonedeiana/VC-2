...
template<int w, int h, int d, int QUAL, int passes, class T> void encode_slices(CodedSlice<T> *slices, int n, QuantisationMatrices *matrices, int encode_length, int wavelet_index, int slice_size_scalar, int, int, int) {
  ...
  std::array<int, passes> n_tgt_slices = {};
  std::array<std::vector<CodedSlice<T> *>, passes> tgt_slices;
  for (auto &pass_slices : tgt_slices)
    pass_slices.reserve(n);
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (passes == 1) {
      ...
      if (coded_size == cl) {
        tgt_slices[0].push_back(&slices[i]);
        ++n_tgt_slices[0];
      }
    }
  ...
  for (int pass = 1; pass < passes; pass++) {
    ...
    for (int i = 0; i < n_tgt_slices[pass - 1]; i++) {
      ...
      if (coded_size == cl) {
        tgt_slices[pass].push_back(tgt_slices[pass - 1][i]);
        ++n_tgt_slices[pass];
      }
    }
  }
