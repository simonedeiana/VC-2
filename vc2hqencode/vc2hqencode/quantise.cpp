...
QuantisationMatrices::QuantisationMatrices(int bw, int bh, int d, int t, int q_min, int q_max) {
  ...
  mQF = (uint16_t *)VC2_ALIGNED_ALLOC(64, (q_max - q_min)*(mMatrixYLen + 2*mMatrixCLen)*sizeof(uint16_t));
  mM  = (uint16_t *)VC2_ALIGNED_ALLOC(64, (q_max - q_min)*(mMatrixYLen + 2*mMatrixCLen)*sizeof(uint16_t));
  mSh = (uint8_t  *)VC2_ALIGNED_ALLOC(64, (q_max - q_min)*(mMatrixYLen + 2*mMatrixCLen)*sizeof(uint8_t));
...
QuantisationMatrices::~QuantisationMatrices() {
  VC2_ALIGNED_FREE(mQF);
  VC2_ALIGNED_FREE(mM);
  VC2_ALIGNED_FREE(mSh);
}
