...
void detect_cpu_features() {
  vc2_detect_cpu_features(HAS_SSE4_2, HAS_AVX, HAS_AVX2);
...
template<class T> void VC2Encoder::EncodePartial(partial_encode_data *data) {
  ...
  /* Serialise */
  {
    vc2hq_stage_profile::Scope profile(vc2hq_stage_profile::SERIALISE);
...
template <class T> void VC2Encoder::Transform(JobBase *_job) {
  ...
  for (int c = 0; c < 3; c++) {
    ...
    if (transforms_v[0]) {
      vc2hq_stage_profile::Scope profile(vc2hq_stage_profile::VERTICAL_WAVELET);
    ...
    for (int l = 1; l < mDepth; l++) {
      skip <<= 1;
      {
        vc2hq_stage_profile::Scope profile(vc2hq_stage_profile::HORIZONTAL_WAVELET);
