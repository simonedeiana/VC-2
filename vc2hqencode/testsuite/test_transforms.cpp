...
#define free(pointer) _aligned_free(pointer)
#else
#include <unistd.h>
#endif
...
int test_transforms(bool HAS_SSE4_2, bool HAS_AVX, bool HAS_AVX2) {
  ...
  {
#ifdef _WIN32
    std::mt19937 generator(0x564332u);
    std::uniform_int_distribution<int> distribution(0, 255);
    for (int offset = 0; offset < ilength; ++offset)
      static_cast<uint8_t *>(idata)[offset] = static_cast<uint8_t>(distribution(generator));
#else
