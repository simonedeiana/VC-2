...
#endif
static int vc2_open(const char *path, int flags) {
  return _open(path, flags | _O_BINARY);
}
static int vc2_open(const char *path, int flags, int mode) {
  return _open(path, flags | _O_BINARY, mode);
}
#define open vc2_open
#else
#include <unistd.h>
#endif
#include <time.h>
...
int main (int argc, char *argv[]) {
  ...
  std::vector<char *> idata(static_cast<size_t>(num_frames), nullptr);
  std::vector<std::array<char *, 3>> ipictures(
      static_cast<size_t>(num_frames * (interlace ? 2 : 1)));
  int istride[3];
  ...
  */
  ...
  while (total_frames_encoded < num_frames) {
    ...
    for (; n < num_output_pictures && total_frames_encoded < num_frames; n++) {
      ...
      r = vc2encode_encode_fragmented_data(encoder, ipictures[n].data(), istride, &o, bytes_per_picture, parse_offset, &parse_offset);
      if (r != VC2ENCODER_OK) {
