...
static inline __m256i dd9i_update8(__m256i a, __m256i b, __m256i c, __m256i d) {
...
static inline __m256i dd9i_predict8(__m256i a, __m256i b) {
  return _mm256_srai_epi32(_mm256_add_epi32(_mm256_add_epi32(a, b), _mm256_set1_epi32(2)), 2);
}

static inline void dd9i_store8(int16_t *p, __m256i even, __m256i odd, __m128i lo16) {
...
void Deslauriers_Dubuc_9_7_transform_H_inplace_10P2_avx2(const char *_idata,
                                                         ...
                                                         const int owidth,
                                                         const int oheight) {
  // The common 1920/960-wide 10P2 inputs have eight even samples per AVX2
  // vector. Keep the existing four-lane path below for odd multiples of
  // eight so that non-standard geometry retains its previous behavior.
  if (iwidth >= 16 && (iwidth & 15) == 0 && iheight >= 8) {
    ...
    for (int y = 0; y < iheight; y++) {
      ...
      for (int j0 = 0; j0 < nE; j0 += 8) {
        ...
        _mm256_storeu_si256((__m256i *)(Op + j0), _mm256_sub_epi32(odd, dd9i_update8(s1, s2, s3, d_hi)));
      }
      Op[-1] = Op[0];
      ...
      for (int j0 = 0; j0 < nE; j0 += 8) {
        ...
        dd9i_store8(orow + 2 * j0, _mm256_add_epi32(ev, dd9i_predict8(prev, cur)), cur, LO16);
      }
    }
